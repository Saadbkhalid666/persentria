...
const TYPE_STYLES = {
  VEHICLE_DETECTED: { dot: 'bg-cyan-400', text: 'text-cyan-400' },
  VEHICLE_RECOGNIZED: { dot: 'bg-emerald-400', text: 'text-emerald-400' },
      ...
      <div className="flex-1 overflow-y-auto space-y-1 font-mono text-[10px] custom-scroll pr-1">
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-700">
            ...
            <p className="text-center">No vehicle events yet.<br />Start camera feed or run a directory scan.</p>
          </div>
        ) : (
          events.map((ev) => {
