...
def draw_cars(frame, cars, vehicle_results):
    for car in cars:
        ...
        if result:
            label = " | ".join(line.strip() for line in result.splitlines() if line.strip())
            if len(label) > 100:
                label = label[:100] + "..."
            cv2.putText(
                frame,
...
def main():
    ...
    results = process_frame(frame, timestamp_ms=0, reset_state=True)
    cars = results["cars"]
    ...
    for car in cars:
        ...
        try:
            vehicle_result = recognize_vehicle(image_base64)
    ...
    draw_cars(frame, cars, vehicle_results)
    cv2.imshow("Persentria - Vehicle Image Recognition", frame)
